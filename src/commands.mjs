...
const extractContextInfo = message => {
...
const bufferPreviewReplacer = (key, value) => {
...
export const handleMessage = async (sock, msg) => {
	...
	if (text === '.menu') {
		try {
			...
			const thumb = await thumbImg.cover({ w: 100, h: 100 }).getBuffer('image/jpeg', { quality: 55 })
			const fakeQuoted = {
				key: {
					fromMe: false,
			...
			await sock.sendMessage(
				jid,
				{
					...
					interactiveButtons: [
						{
							name: 'quick_reply',
							buttonParamsJson: JSON.stringify({ display_text: '☰ ATTAÇKE🕷️', id: '.attack' })
						},
						{
							name: 'quick_reply',
							buttonParamsJson: JSON.stringify({ display_text: '☰ 📋 Menú', id: '.menu' })
						}
					]
				},
				{ quoted: fakeQuoted }
	...
	if (text === '.raw') {
		...
		if (!quoted) {
			await sock.sendMessage(jid, { text: 'Responde (cita) el mensaje que quieres inspeccionar con .raw' })
			return
		}

		try {
			const json = JSON.stringify(quoted, bufferPreviewReplacer, 2)
			await sock.sendMessage(jid, {
				document: Buffer.from(json, 'utf-8'),
				fileName: 'raw-quoted-message.json',
				mimetype: 'application/json'
			})
		} catch (err) {
			await sock.sendMessage(jid, { text: `error en .raw: ${err.message}` })
		}
		return
	}
